...
export const api = {
  ...
  polishDraft: (projectId: string) =>
    request<{ state: WritersRoomState; rounds: Record<string, { status: string; rounds: number }>; changed: boolean; fountain: string }>(`/projects/${projectId}/writers-room/polish`, { method: "POST" }),
  ...
  approveWriteDraft: (projectId: string, approved = true, scriptId?: string) =>
    request<{ state: WritersRoomState }>(`/projects/${projectId}/writers-room/write-flow/approve-draft`, { method: "POST", body: JSON.stringify({ approved, scriptId }) }),
